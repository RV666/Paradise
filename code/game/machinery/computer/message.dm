			...
			for(var/obj/machinery/message_server/server in GLOB.message_servers)
				if(!isnull(server))
					if(!isnull(server.decryptkey))
						info = "<center><h2>Ежедневный Сброс Ключа</h2></center><br>Новый ключ монитора сообщений '[server.decryptkey]'.<br>Пожалуйста, держите это в секрете и подальше от клоуна.<br>При необходимости измените пароль на более безопасный."
						info_links = info
