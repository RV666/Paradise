...
/datum/disease/critical/shock/stage_act()
	if(..())
		if(affected_mob.health >= 25 && affected_mob.nutrition >= NUTRITION_LEVEL_HYPOGLYCEMIA)
			to_chat(affected_mob, "<span class='notice'>Ты чувствуешь себя лучше.</span>")
			cure()
			return
		switch(stage)
			if(1)
				...
				if(prob(5))
					to_chat(affected_mob, "<span class='danger'>Ты чувствуешь слабость!</span>")
			if(2)
				if(prob(1) && prob(10))
					to_chat(affected_mob, "<span class='notice'>Ты чувствуешь себя лучше.</span>")
					cure()
				...
				if(prob(5))
					to_chat(affected_mob, "<span class='danger'>Ты чувствуешь себя абсолютно ужасно!</span>")
				if(prob(5))
			...
			if(3)
				...
				if(prob(5))
					to_chat(affected_mob, "<span class='danger'>Ты чувствуешь себя ужасно!</span>")
				if(prob(5))
					affected_mob.emote(pick("faint", "collapse", "groan"))
				if(prob(7))
					to_chat(affected_mob, "<span class='danger'>Ты не можешь дышать!</span>")
	...
	if(..())
		switch(stage)
			if(1)
				...
				if(prob(5))
					to_chat(affected_mob, "<span class='danger'>Твоя рука болит!</span>")
				else if(prob(5))
					to_chat(affected_mob, "<span class='danger'>Твоя грудь болит!</span>")
			if(2)
				if(prob(1) && prob(10))
					to_chat(affected_mob, "<span class='notice'>Ты чувствуешь себя лучше.</span>")
					cure()
				...
				if(prob(5))
					...
					affected_mob.AdjustLoseBreath(1)
				if(prob(3))
					to_chat(affected_mob, "<span class='danger'>Твое сердце остановилось!</span>")
